...
that specify how can be /reduced/ to a normal form. Moreover,
expressions in a pure fragment of the language evaluate to canonical
forms. For example, every closed term of type =ℕ= evaluates to a
...
bytecode evaluator usses the Gnu multiprecision library to carry out
...
- [[https://github.com/leanprover/theorem_proving_in_lean][Theorem Proving in Lean]]
