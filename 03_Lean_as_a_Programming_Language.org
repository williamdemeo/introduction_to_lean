...
Calculus of Inductive Constructions, though in the case of =fact= the
...
vm_eval fib 10
#+END_SRC
The naive implementation is inefficient : the computation of =fib
(n+2)= results in two independent computations of =fib n=, one hidden
in the computation of =fib (n+1)=. As a result, the run time is
exponential in =n=. A more efficient way to go is to define an
auxiliary function that computes the values in pairs:
#+BEGIN_SRC lean
def fib_aux : ℕ → ℕ × ℕ
| 0     := (0, 1)
...
# TODO(Jeremy): can we profile the results?
# TODO(Jeremy): make sure the first fib 100 is not too slow in the
# Javascript version
