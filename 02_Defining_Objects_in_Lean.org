...
depends on a construct known as /Hilbert's epsilon/. 

The next two definitions have essentially the same effect, although they 
do not specify the default value in case the given condition fails:
#+BEGIN_SRC lean
open classical
local attribute [instance] prop_decidable
